...
ap.add_argument("-y", "--display", type = int, default = 1, help = "whether or not to display frame to screen")
...
# initialise the video stream and pointer to output video file, then
...
while True:
    ...
    # to have a width f 750px(to speedup processing)
    ...
    for encoding in encodings:
        ...
        if True in matches:
            ...
            # was matched
            matchedIdxs = [ i for (i,b) in enumerate(i, b) if b]
            counts = {}
    ...
    for ((top, right, bottom, left), name) in zip(boxes, names):
        # rescale the top coordinates
        top = int(top * r)
        right = int(right * r)
        bottom = int(bottom * r)
        left = int(left * r)

        # draw the predicted name of the image
    ...
    if writer is not None and args["output"] is not None:
        fourcc = cv2.VideoWriter_fourcc(*"MJPG")
        writer = cv2.VideoWriter(args["output"], fourcc, 20, (frame.shape[1], frame.shape[0]), True)

    # if the writer is not None, write the frame with recognized faces 
    # to disk
